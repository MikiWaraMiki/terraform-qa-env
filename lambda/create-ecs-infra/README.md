...
```bash
create-ecs-infra$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
create-ecs-infra$ sam local start-api
create-ecs-infra$ curl http://localhost:3000/
```
...
```bash
create-ecs-infra$ sam logs -n HelloWorldFunction --stack-name create-ecs-infra --tail
```
...
```bash
create-ecs-infra$ cd hello-world
...
```bash
aws cloudformation delete-stack --stack-name create-ecs-infra
```
